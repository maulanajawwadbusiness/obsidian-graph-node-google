...
export const applyDenseCoreVelocityDeLocking = (
    ...
    for (const node of nodeList) {
        ...
        if (neighborCount < densityThreshold) continue;
        ...
        const vParallelMag = node.vx * avgUx + node.vy * avgUy;
        const vParallelX = vParallelMag * avgUx;
        const vParallelY = vParallelMag * avgUy;

        // Perpendicular component
        const vPerpX = node.vx - vParallelX;
        const vPerpY = node.vy - vParallelY;

        // Recombine with reduced parallel (micro-slip)
        const beforeVx = node.vx;
        const beforeVy = node.vy;

        node.vx = vPerpX + vParallelX * (1 - parallelReduction);
        node.vy = vPerpY + vParallelY * (1 - parallelReduction);
    ...
    if (affected.size > 0) {
        console.log(`[VelocityDeLocking] affected: ${affected.size} nodes`);
    }
};
...
export const applyStaticFrictionBypass = (
    ...
    for (const link of engine.links) {
        ...
        if (sourceDensity < densityThreshold && targetDensity < densityThreshold) continue;

        // Compute relative velocity
        const relVx = source.vx - target.vx;
        const relVy = source.vy - target.vy;
        const relVMag = Math.sqrt(relVx * relVx + relVy * relVy);
    ...
    // DEBUG
    if (affected.size > 0) {
        console.log(`[StaticFrictionBypass] unlocked: ${affected.size} nodes`);
    }
};
